...
public class ClientRest {
    ...
    @ApiOperation("Search top 5 client in period")
    @GetMapping("/startDate/{start}/endDate/{end}")
    public List<ClientVo> findTopfiveClient(@PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) Date start,
                                            @PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) Date end ){
    	return clientService.findTopfiveClient(start,end);
    }
